...
import org.yawl.schedulingservice.client.util.CronEntity;
import org.yawl.schedulingservice.client.util.CronExpressionParser;
...
public class ScheduleView extends Composite {
	...
	ScheduleView() {
		...
		horizontalPanel = new HorizontalPanel();
		horizontalPanel
		        .add(new Label(constants.label_custom_cron_expression()));
		horizontalPanel.add(txtCronExpression);
		txtAutoCronExpression.setEnabled(false);
		horizontalPanel.add(new Label(constants
	...
	private void enableNumericOnly() {
		WidgetUtil.allowNumericOnly(txtSeconds, false);
		WidgetUtil.allowNumericOnly(txtMinutes, false);
		WidgetUtil.allowNumericOnly(txtHours, false);
		WidgetUtil.allowNumericOnly(txtDays, false);
		WidgetUtil.allowNumericOnly(txtMonths, false);
		
		WidgetUtil.allowNumericOnly(txtAtSeconds, false);
		WidgetUtil.allowNumericOnly(txtAtMinutes, false);
		WidgetUtil.allowNumericOnly(txtAtHours, false);
		
		WidgetUtil.allowNumericOnly(txtMonthDay, false);
	}
	...
	private boolean anyWeekDaySelected() {
		return ((chkSun.getValue() || chkMon.getValue() || chkTue.getValue()
		        || chkWed.getValue() || chkThu.getValue() || chkFri.getValue() || chkSat
		        .getValue()) && rdWeekDay.getValue());
	}
