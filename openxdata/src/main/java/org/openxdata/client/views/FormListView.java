		...
		Scheduler.get().scheduleDeferred(new ScheduledCommand() {
			...
			public void execute() {
				...
				if (showAllFormVersions) {
					showAllFormVersions = false;
					allVersions.setText(appMessages.showPublishedVersions());
					// TODO: implement fetching of all versions (or merely
					// showing hidden versions)
				} else {
					showAllFormVersions = true;
					allVersions.setText(appMessages.showAllVersions());
					// TODO: implement hiding of all versions
				}
			}
		});
		ProgressIndicator.hideProgressBar();
