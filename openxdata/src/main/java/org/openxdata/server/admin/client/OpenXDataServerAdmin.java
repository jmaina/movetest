			        ...
			        public void onSuccess(User user) {
				        if (user != null) {
					        if (!user.isDisabled()) {
						        ...
						        Utilities
						                .displayMessage("You account has been disabled. Contact the system administrator.");
					        }
				        } else
			...
			public void execute() {
				try {
					...
					if (mainView == null) {
						...
						mainView = (MainView) widgetFactory
						        .getMainView(logoutListener);
					...
					Context.getUserService().getUser("admin",
					        new OpenXDataAsyncCallback<User>() {
						        ...
						        public void onOtherFailure(Throwable throwable) {
							        Utilities.displayMessage(throwable
							                .getLocalizedMessage());
						        }
		...
		Scheduler.get().scheduleDeferred(new ScheduledCommand() {
			...
			public void execute() {
				if (!passwordChanged) {
					...
					Utilities
					        .displayMessage("For security reasons, the administrator should change their password on initial login. "
