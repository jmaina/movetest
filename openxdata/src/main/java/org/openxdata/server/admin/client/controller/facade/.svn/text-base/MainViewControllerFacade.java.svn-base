...
import org.openxdata.server.admin.client.view.event.ViewMatchWorkflowsEvent;
...
public class MainViewControllerFacade implements StackPanelListener {
	 ...
	 * 
	 * @throws OpenXDataException
	 *             For any <code>Exception</code> that might occur during
	 *             saving.
	 */
	 ...
	 */
	public static void prepareMainViewController(Composite mainView,
	        OpenXDataBaseTreeView studiesTreeView,
		...
		if (getMVCInstance() == null)
			mainViewControllerInstance = new MainViewController(
			        studiesTreeView, usersTreeView, rolesTreeView,
			        tasksTreeView, settingsTreeView, reportsTreeView,
			        studyView, userView, roleView, reportView);
	}
	...
	public static void loadPreliminaryViewData() {
		...
		Scheduler.get().scheduleDeferred(new ScheduledCommand() {
			...
			public void execute() {
				...
				((OpenXDataToolBar) widgetFactory.getOpenXDataToolBar())
				        .refresh();
		...
		Scheduler.get().scheduleDeferred(new ScheduledCommand() {
			...
			public void execute() {
				switch (widgetFactory.getOpenXdataStackPanel()
				        .getSelectedIndex()) {
					case OpenXDataStackPanelConstants.INDEX_STUDIES:
						if (((StudyView) widgetFactory.getStudyView())
						        .isInFormDesignMode())
							((StudyView) widgetFactory.getStudyView())
							        .refreshItem();
						else {
					...
					case OpenXDataStackPanelConstants.INDEX_SETTINGS:
						...
						break;
					case OpenXDataStackPanelConstants.INDEX_REPORTS:
						if (((ReportView) widgetFactory.getReportView())
						        .isInReportDataMode())
							((ReportView) widgetFactory.getReportView())
							        .refresh();
						else {
							if (RolesListUtil.getPermissionResolver()
					...
					case OpenXDataStackPanelConstants.INDEX_REPORTS:
						...
						break;
                               ...
                               case OpenXDataStackPanelConstants.INDEX_WORKFLOWs:
                                       //Important to instatiate the view first before u fire the event
                                        widgetFactory.getHorizontalSplitPanel().setRightWidget(widgetFactory.getWorkFlowView());
                                        widgetFactory.getEventBus().fireEvent(new ViewMatchWorkflowsEvent());
                                        break;
				}
