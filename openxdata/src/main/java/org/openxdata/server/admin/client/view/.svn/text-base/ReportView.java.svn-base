...
public class ReportView extends OpenXDataBaseView implements
	 ...
	 */
	public ReportView(OpenXDataViewFactory openXDataViewFactory) {
	...
	private void loadView() {
		...
		/*
		 * lbType.addItem("Area Chart"); lbType.addItem("Tube Chart");
		 * lbType.addItem("Cone Chart"); lbType.addItem("Pyramid Chart");
		 * lbType.addItem("Scatter Chart"); lbType.addItem("Stock Chart");
		 * lbType.addItem("Bubble Chart"); lbType.addItem("Difference Chart");
		 */
		...
		txtDefinition.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent arg0) {
				if (report != null) {
					...
					eventBus.fireEvent(new ItemChangedEvent(report));
				}
			}
		});
	...
	private void updateName() {
		if (report != null) {
			...
			reportGroup.setName(txtName.getText());
                        eventBus.fireEvent(new ItemChangedEvent(reportGroup));
		}
	}
	...
	private void bindHandlers() {
		ItemSelectedEvent.HandlerAdaptor handler = new ItemSelectedEvent.HandlerAdaptor() {
			...
			public void onReportSelected(Composite sender, Report report) {
				onItemSelected(sender, report);
			}
			
			@Override
			public void onReportGroupSelected(Composite composite,
			        ReportGroup reportGroup) {
				onItemSelected(composite, reportGroup);
			}
		};
