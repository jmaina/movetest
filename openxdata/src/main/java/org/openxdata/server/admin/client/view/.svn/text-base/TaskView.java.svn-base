...
import org.openxdata.server.admin.client.view.event.ItemSelectedEvent.HandlerAdaptor;
...
public class TaskView extends OpenXDataBaseView {
	 ...
	 */
	@Inject
	public TaskView(TasksTreeView itemChangeListener,
	...
	private void clear() {
        ...
        private void initHandlers() {
                HandlerAdaptor handler = new ItemSelectedEvent.HandlerAdaptor() {

                        @Override
                        public void onTaskDefSelected(Composite sender, TaskDef taskDef) {
                                onItemSelected(sender, taskDef);
                        }
                };
                eventBus.addHandler(ItemSelectedEvent.TYPE, handler);
                TaskDefEvent.Handler taskDefHandler = new TaskDefEvent.HandlerAdaptor() {

                        @Override
                        public void onDeleted(TaskDef taskDf) {
                                if (taskDef.equals(taskDf))
                                        onItemSelected(null, null);
                        }
                };
                eventBus.addHandler(TaskDefEvent.TYPE, taskDefHandler);
        }
