...
import org.openxdata.server.admin.client.view.event.ItemChangedEvent;
...
/**
 * Displays properties of the selected <tt>User</tt> and allows you to
 * manipulate them.
 * 
 * @author angel
 * @author daniel
 ...
 * 
 */
public class UserView extends OpenXDataBaseView implements StudiesObserver,
        ReportsObserver {
	 ...
	 *            itemChangeListener listener to <tt>User</tt> property changes.
	 * @param openXDataViewFactory
	 */
	public UserView(OpenXDataWidgetFactory openXDataViewFactory) {
	...
	private void loadView() {
		...
		if (RolesListUtil.getPermissionResolver().isAddPermission(
		        Permission.PERM_ADD_USERS)) {
			
			if (RolesListUtil.getPermissionResolver().isAddPermission(
			        Permission.PERM_ADD_ROLES)) {
				setupUserRoleMapView();
			}
			
			if (RolesListUtil.getPermissionResolver().isAddPermission(
			        Permission.PERM_ADD_STUDIES)) {
			...
			if (RolesListUtil.getPermissionResolver().isAddPermission(
			        Permission.PERM_ADD_REPORTGROUPS)) {
				setupUserReportGroupMapView();
			}
			
			if (RolesListUtil.getPermissionResolver().isAddPermission(
			        Permission.PERM_ADD_REPORTS)) {
	...
	private void setupUserReportMapView() {
		...
		widgetFactory.getUserReportMapView().setDeletedUserMappedReports(
		        deletedUserMappedReports);
	...
	private void setupUserReportGroupMapView() {
		...
		widgetFactory
		        .getUserReportGroupMapView()
		        .setDeletedUserMappedReportGroups(deletedUserMappedReportGroups);
		
		tabs.add(widgetFactory.getUserReportGroupMapView(),
		        "User Report Group Permissions");
	...
	private void setupUserFormMapView() {
		...
		widgetFactory.getUserFormMapView().setDeletedUserMappedForms(
		        deletedUserMappedForms);
	...
	private void setupUserStudyMapView() {
		...
		widgetFactory.getUserStudyMapView().setDeletedUserMappedStudies(
		        deletedUserMappedStudies);
	...
	private void loadPermissionLessView() {
		table.setWidget(0, 0,
		        new Label(constants.ascertain_permissionLessView() + "Users"));
		FlexCellFormatter cellFormatter = table.getFlexCellFormatter();
		cellFormatter.setWidth(0, 0, "20%");
	 ...
	 * @return -
	 *         <code>True If(txtPassword.getText().Length() > 6(or default Password Length as set by administrator)</code>
	 *         <p>
	 *         <code>else false
	 *         </p>
	 *         </code>
	...
	private void validatePasswordEquality() {
		if (!txtPassword.getText().equals(txtConfirmPassword.getText())) {
			Utilities
			        .displayMessage("The Passwords entered do not match! Please check and rectify to proceed!");
	...
	/**
	 * Member that updates the name of the user on the display (tree view) as it
	 * changes The changes are also captured and stored on the object
	 */
	private void updateName() {
		if (txtName.isVisible()) {
			user.setName(txtName.getText());
			eventBus.fireEvent(new ItemChangedEvent(user));
		}
	}
	
	/**
	 * Member that updates the phone number of the user as it changes on the
	 * display
	 */
	...
	private void updateEmail() {
		if (!Utilities.validateEmail(txtEmail.getText())) {
			String message = "The email address is not a valid. Check to see if it is of the required pattern.";
			Utilities.displayMessage(message);
		} else {
	...
	private void updateUserStatus() {
		...
		if (user != null) {
			if (user.isDefaultAdministrator()) {
				Utilities
				        .displayMessage("You cannot disable the default Administrator!");
				cboStatus.setSelectedIndex(0);
			} else if (Utilities.isUserGuyzb(user.getName())) {
				
				if (cboStatus.getItemText(cboStatus.getSelectedIndex()).equals(
				        "Active")) {
					Utilities
					        .displayMessage("You are enabling guyzb. This User ships with the system and his password is known by the community. "
	...
	private void setStatus() {
		user.setStatusAfterGiveType(cboStatus.getItemText(cboStatus
		        .getSelectedIndex()));
		eventBus.fireEvent(new ItemChangedEvent(user));
	}
...
//		notifyItemSelectionListeners(sender, user);
	...
	/**
	 * Predicate member that clears the contents in the text boxes on the
	 * display
	 */
	private void clear() {
	 ...
	 * 
	 * @param enabled
	 *            - parameter to enable the control. True for enabling,
	 *            otherwise disable
	 */
	...
	/**
	 * @see com.google.gwt.user.client.ui.TabListener#onBeforeTabSelected(int)
	 */
	 ...
	 *            <code>List</code> of <code>User Mapped Studies.</code>
	 */
	public void setUserMappedStudies(List<UserStudyMap> userMappedStudies) {
		this.userMappedStudies = userMappedStudies;
		
		widgetFactory.getUserFormMapView().setUserMappedStudies(
		        userMappedStudies);
		widgetFactory.getUserStudyMapView().setUserMappedStudies(
		        userMappedStudies);
		widgetFactory.getUserStudyMapView().initializeObjectProperties();
	}
	 ...
	 * 
	 * @param userMappedForms
	 *            <code>List</code> of <code>User Mapped Forms.</code>
	 */
	public void setUserMappedForms(List<UserFormMap> userMappedForms) {
		this.userMappedForms = userMappedForms;
		widgetFactory.getUserFormMapView().setUserMappedForms(userMappedForms);
		widgetFactory.getUserFormMapView().initializeObjectProperties();
	}
	 ...
	 *            <code>List</code> of <code>User Mapped Report Groups.</code>
	 */
	public void setUserMappedReportGroups(
	        List<UserReportGroupMap> userMappedReportGroups) {
		this.userMappedReportGroups = userMappedReportGroups;
		widgetFactory.getUserReportGroupMapView().setUserMappedReportGroups(
		        userMappedReportGroups);
		widgetFactory.getUserReportGroupMapView().initializeObjectProperties();
	}
	 ...
	 *            <code>List</code> of <code>Mapped Reports.</code>
	 */
	public void setUserMappedReports(List<UserReportMap> userMappedReports) {
		this.userMappedReports = userMappedReports;
		widgetFactory.getUserReportMapView().setUserMappedReports(
		        userMappedReports);
		widgetFactory.getUserReportMapView().initializeObjectProperties();
	}
	 ...
	 * 
	 * @return <code>List</code> of deleted </code>User Mapped Report
	 *         Groups.</code>
	 */
	...
	private void bindHandlers() {
		ItemSelectedEvent.HandlerAdaptor handler = new ItemSelectedEvent.HandlerAdaptor() {
		...
		eventBus.addHandler(ItemSelectedEvent.TYPE, handler);
	}
}
