...
public class RoleView extends OpenXDataBaseView implements RolesObserver {
	 ...
	 *            <tt>ItemChangeListener</tt> sending objects that this
	 *            <tt>Widget</tt> is interest in.
	 * @param openXDataViewFactory
	 */
	public RoleView(OpenXDataViewFactory openXDataViewFactory) {
	...
	private void loadView() {
		...
		if (RolesListUtil.getPermissionResolver().isEditPermission(
		        "Perm_Edit_Role_Permissions")) {
			tabs.add(widgetFactory.getRolePermissionMapView(),
			        constants.label_rolesmanagement());
		}
	...
	private void loadPermissionLessView() {
		table.setWidget(0, 0,
		        new Label(constants.ascertain_permissionLessView() + "Roles"));
		FlexCellFormatter cellFormatter = table.getFlexCellFormatter();
		cellFormatter.setWidth(0, 0, "20%");
	...
	/**
	 * Member that updates the name of the role on the display (tree view) as
	 * changes The changes are also captures and stored on the object
	 */
	private void updateName() {
		
		if (txtName.isVisible()) {
			role.setName(txtName.getText());
			eventBus.fireEvent(new ItemChangedEvent(role));
		}
	}
	...
	public void onItemSelected(Composite sender, Object item) {
		...
		if (item == null)
			...
			widgetFactory.getRolePermissionMapView().onItemSelected(sender,
			        item);
		}
	}
	
	/**
	 * Disables controls if the selected <tt>Role</tt> is the default
	 * administrator <tt>Role</tt> that ships with the system.
	 */
	 ...
	 * 
	 * It also sets the currently selected member to the just selected member on
	 * the display
	 * 
	 * @param item
	 *            - currently selected item on the tree view
	 */
	...
	@Override
	public void updatePermissions(OpenXDataObservable observable,
	        List<Permission> permissions) {
		((RolePermissionMapView) widgetFactory.getRolePermissionMapView())
