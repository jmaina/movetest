...
/**
 * Custom <code>widget</code> that encapsulates operations that accomplish
 * mapping of <code>Permissions to Roles.</code>
 * 
 * @author Angel
 * 
 */
public class RolePermissionMapView extends OpenXDataBaseObjectMapView {
	 ...
	 *            Event Listener.
	 */
	public RolePermissionMapView(EventBus eventBus) {
		super(eventBus);
	}
	...
	protected void initializeObjectProperties() {
		...
		if (rolePermissions.size() > 0 && permissions != null) {
			for (Permission permission : permissions) {
				...
				// objects list box else add it to the unmapped objects list
				// box.
	...
	protected void mapItemTobject() {
		if (unMappedObjectsListBox.getSelectedIndex() > -1) {
			for (Permission x : permissions) {
				if (x.getName().equals(
				        unMappedObjectsListBox
				                .getItemText(unMappedObjectsListBox
				                        .getSelectedIndex()))) {
					if (role != null) {
						if (!role.isDefaultAdminRole()) {
							List<Permission> permissionsToAdd = RolesListUtil
							        .checkAndBindAncillaryPermissions(x,
							...
							if (permissionsToAdd != null
								...
								unMappedObjectsListBox
								        .removeItem(unMappedObjectsListBox
								                .getSelectedIndex());
							...
							Utilities
							        .displayMessage("You cannot edit the Default Administrator Role!");
						}
					} else {
						if (unMappedObjectsListBox.getItemCount() < 0)
							Utilities
							        .displayMessage("There are no Permissions to map to the selected Role. Either they have all been mapped to the Role "
							                + "or there are no Permissions in the System.");
						else
							Utilities
							        .displayMessage("Select a Permission to Proceed!");
					}
				}
			}
		}
	...
	protected void unMapItemFromObject() {
		if (mappedObjectsListBox.getSelectedIndex() > -1) {
			for (Permission x : permissions) {
				if (x.getName().equals(
				        mappedObjectsListBox.getItemText(mappedObjectsListBox
				                .getSelectedIndex()))) {
					if (role != null) {
						...
						mappedObjectsListBox.removeItem(mappedObjectsListBox
						        .getSelectedIndex());
						break;
					} else {
			...
			Utilities
			        .displayMessage("The selected Role has no Permissions mapped to it!");
		}
