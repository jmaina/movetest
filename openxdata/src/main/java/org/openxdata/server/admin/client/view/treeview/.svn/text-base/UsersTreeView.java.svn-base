 ...
 * <code>system users</code> to the <code>tree view</code> and renders them for
 * display.
 * 
 * Listens for actions on the <code>widgets</code>. Forwards some of the actions
 * on the roles to the relevant listeners
 * 
 * @author daniel
 * @author Angel
 * @author Ronald.K
 */
public class UsersTreeView extends OpenXDataBaseTreeView implements
        UsersObserver, OpenFileDialogEventListener,
	 ...
	 * 
	 * @param images
	 *            - images to be bound to the tree view
	 * @param permissionResolver
	 *            - permissionLisUtil to manage user permissions
	 */
	 ...
	 * 
	 * @param users
	 *            - List of users that are in the system
	 */
	public void bindUsersToTreeView(List<User> users) {
		...
		if (isLoadData()) {
			...
			for (int i = 0; i < users.size(); i++) {
				...
				if (user.getUserId() == Context.getAuthenticatedUser()
				        .getUserId()) {
				...
				root = new CompositeTreeItem(new TreeItemWidget(
				        images.lookup(), user.getName(), popup));
	...
	public void deleteSelectedItem() {
		Utilities
		        .displayMessage("You cannot delete Users because the System uses them for other system functions like logging. "
	...
	public void changeEditableProperties(Object item) {
		...
		User user = (User) item;
		{
			...
			treeItem.setWidget(new TreeItemWidget(images.lookup(), user
			        .getName(), popup));
		...
		try {
			...
			for (int j = 0; j < index; j++) {
				if (map.containsKey(tree.getItem(j).getText().toLowerCase())) {
					...
					Window.alert(constants.label_existinguser()
	...
	public void onSave() {
        ...
        private void bindHandlers() {
                ItemChangedEvent.Handler handler = new ItemChangedEvent.HandlerAdaptor() {
                ...
                eventBus.addHandler(ItemChangedEvent.TYPE, handler);
        }
}
