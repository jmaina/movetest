...
/**
 * Custom widget that binds a list of system roles to the tree view and renders
 * them for display
 * 
 * Listens for actions on the widgets Forwards some of the actions on the roles
 * to relevant listeners
 * 
 * @author Angel
 * 
 */
public class RolesTreeView extends OpenXDataBaseTreeView implements
        OpenXDataViewApplicationEventListener, RolesObserver {
	 ...
	 * 
	 * @param roles
	 *            - list of roles that are in the system
	 */
		...
		if (isLoadData()) {
			...
			for (int i = 0; i < roles.size(); i++) {
				...
				root = new CompositeTreeItem(new TreeItemWidget(
				        images.lookup(), role.getName(), popup));
	...
	public void addNewItem() {
		Role role = new Role(constants.label_newrole());
	...
	public void deleteSelectedItem() {
		...
		if (roleToDelete == null) {
			Window.alert(constants.label_nullrole());
			return;
		}
	...
	private void deleteSelectedRole() {
		...
		if (!Window.confirm(constants.label_deleteroleconfirmation()))
			return;
	...
	public void changeEditableProperties(Object item) {
		...
		
		{
			...
			treeItem.setWidget(new TreeItemWidget(images.lookup(), role
			        .getName(), popup));
		...
		try {
			...
			for (int j = 0; j < index; j++) {
				if (map.containsKey(tree.getItem(j).getText().toLowerCase())) {
					...
					Window.alert(constants.label_existingrole()
	...
	UIViewLabels getContextMenuLabels() {
		...
		labels.setAddLabel(constants.label_addnewrole());
		labels.setDeleteLabel(constants.label_deleterole());
		return labels;
	}
	...
	public void onSave() {
        ...
        private void initHandler() {
                ItemChangedEvent.Handler itemChangeHandler = new ItemChangedEvent.HandlerAdaptor(){

                        @Override
                        public void onRoleChanged(Composite sender, Role role) {
                          changeEditableProperties(role);
                        }

                };
                eventBus.addHandler(ItemChangedEvent.TYPE, itemChangeHandler);
        }
}
