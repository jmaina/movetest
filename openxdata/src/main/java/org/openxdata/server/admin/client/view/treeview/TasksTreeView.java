...
import org.openxdata.server.admin.client.view.event.TaskDefEvent.HandlerAdaptor;
...
import org.openxdata.server.admin.client.view.event.TaskDefEvent;
 ...
 * 
 */
public class TasksTreeView extends OpenXDataBaseTreeView implements
        OpenXDataViewApplicationEventListener, TasksObserver {
	...
	protected PopupPanel initContextMenu() {
		...
		if (popup != null) {
			...
			if (menuBar != null) {
				...
				mnuStart = menuBar.addItem(
				        Utilities.createHeaderHTML(images.play(),
				                constants.label_start()), true, new Command() {
					        @Override
					        public void execute() {
						        popup.hide();
						        startTask();
				...
				menuBar.addSeparator();
				mnuStop = menuBar.addItem(
				        Utilities.createHeaderHTML(images.stop(),
				                constants.label_stop()), true, new Command() {
					        @Override
					        public void execute() {
						        popup.hide();
						        stopTask();
		...
		if (isLoadData()) {
			...
			for (int i = 0; i < tasks.size(); i++) {
				...
				root = new CompositeTreeItem(new TreeItemWidget(
				        images.lookup(), task.getName(), popup));
	 ...
	 */
        ...
        public void deleteSelectedItem() {
                ...
                if (item == null) {
                        Window.alert(constants.label_deletetaskselect());
                        return;
                }

                if (!Window.confirm(constants.label_deletetaskconfirmation()))
                        return;
                TaskDef taskDef = (TaskDef) item.getUserObject();
                deletedTasks.add(taskDef);
                tasks.remove(taskDef);
                Utilities.removeRootItem(tree, item);
                eventBus.fireEvent(new TaskDefEvent(taskDef, EventType.DELETED));
        }
	...
	public void changeEditableProperties(Object item) {
		...
		treeItem.setWidget(new TreeItemWidget(images.lookup(), task.getName(),
		        popup));
	...
	public boolean isValidTasksList() {
		...
		for (int j = 0; j < index; j++) {
			if (map.containsKey(tree.getItem(j).getText().toLowerCase())) {
				...
				Window.alert(constants.label_existingtask()
	...
	private void startTask() {
		if (!Window.confirm(constants.label_starttask()))
			return;
		
		MainViewControllerFacade.startTask((TaskDef) tree.getSelectedItem()
		        .getUserObject());
	}
	...
	private void stopTask() {
		...
		MainViewControllerFacade.stopTask((TaskDef) tree.getSelectedItem()
		        .getUserObject());
	}
	...
	private void updateTaskListeners(TaskDef taskDef) {
		...
		eventBus.fireEvent(new TaskDefEvent(taskDef));
	}
	
	@Override
	UIViewLabels getContextMenuLabels() {
		UIViewLabels labels = new UIViewLabels();
		labels.setAddLabel(constants.label_addnewtask());
		labels.setDeleteLabel(constants.label_deletetask());
		return labels;
	}
	...
	public void onSave() {
        ...
        private void initHandlers() {
                HandlerAdaptor handler = new TaskDefEvent.HandlerAdaptor() {

                        @Override
                        public void onChanged(TaskDef taskDef) {
                                changeEditableProperties(taskDef);
                        }
                };
                eventBus.addHandler(TaskDefEvent.TYPE, handler);
        }
}
